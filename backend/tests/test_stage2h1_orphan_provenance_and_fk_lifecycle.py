...
FK_MIGRATION_PATH = BACKEND_SQL / "stage-29-analysis-artifact-fk-integrity.sql"
SYMPTOMS_MIGRATION_PATH = BACKEND_SQL / "stage-28-symptoms-schema-consolidation.sql"
FK_MIGRATION = FK_MIGRATION_PATH.read_text() if FK_MIGRATION_PATH.exists() else None
SYMPTOMS_MIGRATION = SYMPTOMS_MIGRATION_PATH.read_text() if SYMPTOMS_MIGRATION_PATH.exists() else None
...
def test_no_other_backend_code_path_deletes_lab_uploads_directly():
    ...
    result = subprocess.run(
        ["grep", "-rn", r'\.table("lab_uploads")\.delete', str(BACKEND_APP)],
...
def test_h15_fk_migration_targets_all_four_tables_with_cascade_matching_delete_path():
    if FK_MIGRATION is None:
        pytest.skip("stage-29 FK draft is absent from current production source")
    for table in (
...
def test_h17_symptoms_migration_has_runtime_abort_guard_not_just_a_static_comment():
    if SYMPTOMS_MIGRATION is None:
        pytest.skip("stage-28 symptoms draft is absent from current production source")
...
def test_h18_check_freemium_limit_confirmed_still_dead_zero_callers():
    ...
    result = subprocess.run(
        ["grep", "-rln", "--include=*.py", "check_freemium_limit(",
         str(BACKEND_APP), str(REPO_ROOT / "backend" / "tests")],
        capture_output=True, text=True,
    )
    files_mentioning_it = [
