...
QUESTIONNAIRE_JSX = Path("/var/www/VITALOOP/frontend/src/pages/Questionnaire.jsx").read_text()
DASHBOARD_JSX = Path("/var/www/VITALOOP/frontend/src/pages/UserDashboard.jsx").read_text()
LAB_PLAN_JSX_PATH = Path("/var/www/VITALOOP/frontend/src/pages/LabPlan.jsx")
...
def test_g1_no_other_frontend_file_reads_domain_scores():
    frontend_src = Path("/var/www/VITALOOP/frontend/src")
    hits = []
...
def test_g2_b2c_analyze_router_never_passes_questionnaire_kwarg():
    analyze_source = Path(
        "/var/www/VITALOOP/backend/app/routers/analysis/analyze.py"
    ).read_text()
...
def test_g2_compatibility_router_never_passes_questionnaire_kwarg():
    compat_source = Path(
        "/var/www/VITALOOP/backend/app/routers/protocol/compatibility.py"
    ).read_text()
...
def test_g2_only_b2b_analyze_labs_supplies_a_questionnaire_argument():
    ...
    b2b_source = Path(
        "/var/www/VITALOOP/backend/app/services/b2b/analyze_labs.py"
    ).read_text()
