...
def test_svetlana_case_client_query_staleness_window_is_short(monkeypatch):
    ...
    minutes."""
    source = (FRONTEND_SRC / "hooks/useQueries.js").read_text()
...
def test_login_logout_account_switch_clears_query_cache():
    source = (FRONTEND_SRC / "main.jsx").read_text()
...
def test_client_admin_no_longer_queries_raw_sub_status_directly():
    client_admin = FRONTEND_SRC / "pages/ClientAdmin.jsx"
    if not client_admin.exists():
        pytest.skip("ClientAdmin.jsx is absent from current production source")
    source = client_admin.read_text()
