...
- Backend: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` (or legacy `SUPABASE_SERVICE_KEY`), JWT/Auth settings, `ALLOWED_ORIGINS`, security/rate-limit settings (`SECURITY_ENABLE_HEADERS`, `RATE_LIMIT_TRUST_FORWARDED_FOR`, `RATE_LIMIT_FORWARDED_FOR_HEADER`, `AUTH_RATE_LIMIT_PER_MINUTE`, `ANALYZE_RATE_LIMIT_PER_MINUTE`, `PROTOCOL_RATE_LIMIT_PER_MINUTE`), and retention controls (`LAB_UPLOAD_RAW_RETENTION_DAYS`, `LAB_UPLOAD_RETENTION_BATCH_SIZE`, `RETENTION_ALERT_EMAIL`)
